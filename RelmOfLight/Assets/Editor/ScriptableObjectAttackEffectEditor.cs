...
public class ScriptableObjectAttackEffectEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        origin._Delay = EditorGUILayout.Vector2Field("Delay", origin._Delay);

        if (origin.StatsToAffect != BuffDebuffStatsType.MeleeAttack &&
        ...
        if(origin.OldSystem)
        {
            ...
            else if (origin.StatsToAffect == BuffDebuffStatsType.StealAttack)
            {
                var list = origin.AttackToSteal.ToList();
                int newCount = Mathf.Max(0, EditorGUILayout.IntField("Number of AttackToSteal", list.Count));
                while (newCount < list.Count)
                    list.RemoveAt(list.Count - 1);
                while (newCount > list.Count)
                    list.Add(AttackInputType.Strong);

                for (int i = 0; i < list.Count; i++)
                {
                    list[i] = (AttackInputType)EditorGUILayout.EnumPopup("AttackToSteal " + i, list[i]);

                }
                origin.AttackToSteal = new AttackInputType[list.Count];
                list.CopyTo(origin.AttackToSteal);
            ...
            else if (origin.StatsToAffect == BuffDebuffStatsType.Drain)
            {
                ...
                origin.StatsToAffect == BuffDebuffStatsType.ShadowForm ||
                 origin.StatsToAffect == BuffDebuffStatsType.EndWave ||
        ...
        if (origin.StatsToAffect == BuffDebuffStatsType.MeleeAttack)
        {
            origin.MeleeGoTeleportParticlesOut = (ParticlesType)EditorGUILayout.EnumPopup("MeleeGoTeleportParticlesOut", origin.MeleeGoTeleportParticlesOut);
            origin.MeleeGoTeleportParticlesIn = (ParticlesType)EditorGUILayout.EnumPopup("MeleeGoTeleportParticlesIn", origin.MeleeGoTeleportParticlesIn);
            origin.MeleeBackTeleportParticlesOut = (ParticlesType)EditorGUILayout.EnumPopup("MeleeBackTeleportParticlesOut", origin.MeleeBackTeleportParticlesOut);
            origin.MeleeBackTeleportParticlesIn = (ParticlesType)EditorGUILayout.EnumPopup("MeleeBackTeleportParticlesIn", origin.MeleeBackTeleportParticlesIn);

            origin.MeleeAttackParticles = (ParticlesType)EditorGUILayout.EnumPopup("MeleeAttackParticles", origin.MeleeAttackParticles);
    ...
    public void ShowNewStats(StatsToAffectClass newStats)
    {
        ...
        if (newStats.Show)
        {
            ...
            else if (newStats.StatsToAffect == BuffDebuffStatsType.StealAttack)
            {
                var list = newStats.AttackToSteal.ToList();
                int newCount = Mathf.Max(0, EditorGUILayout.IntField("Number of AttackToSteal", list.Count));
                while (newCount < list.Count)
                    list.RemoveAt(list.Count - 1);
                while (newCount > list.Count)
                ...
                newStats.AttackToSteal = new AttackInputType[list.Count];
                list.CopyTo(newStats.AttackToSteal);
            }
