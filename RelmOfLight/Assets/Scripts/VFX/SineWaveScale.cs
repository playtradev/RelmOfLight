...
public class SineWaveScale : MonoBehaviour
{
    ...
    [Range(0, 1)] public float ScaleAmmount = 0.1f;
    ...
    public struct Cadena
    {
        ...
        public Vector3[] jointsOriginalScale;
        ...
        public void AlimentarJoints()
        {
            ...
            jointsOriginalScale = new Vector3[joints.Length];
            for (int i = 0; i < joints.Length; i++)
            {
                jointsOriginalRot[i] = joints[i].localEulerAngles;
                jointsOriginalScale[i] = joints[i].localScale;
            }
        }
    ...
    void RotarJoint(Cadena _cadena, float time)
    {
        if (_cadena.joints.Length > 0)
            for (int i = 1; i < _cadena.joints.Length * ChainCount; i++)
            {
                ...
                Vector3 scaleOriginal = _cadena.jointsOriginalScale[i];
                _cadena.joints[i].localEulerAngles = new Vector3(0, 0, rotOriginal + angulo);
                if (i == 1)
                    _cadena.joints[i].localScale = new Vector3(scaleOriginal.x + ScaleAmmount * angulo, scaleOriginal.y + ScaleAmmount * angulo, scaleOriginal.z + ScaleAmmount * angulo);
            }
    }
